...
import { searchAiAgentsOrama } from "@/lib/orama-search";

type FilterExpertise = "All" | AiAgentExpertiseLevel;

const expertiseLevels: FilterExpertise[] = ["All", "Beginner", "Intermediate", "Advanced", "Expert"];
...
function useAgentColumns(): number {
...
export function AiAgentsBrowser() {
  ...
  const columnCount = useAgentColumns();
  ...
  useEffect(() => {
    ...
    void searchAiAgentsOrama(normalized, allAgents).then((rankedIds) => {
      if (cancelled) return;
      startRankingTransition(() => {
        setOramaRankedIds(rankedIds);
      });
    });
  ...
  const searchedAgents = useMemo(() => {
    ...
    const filteredById = new Map(filteredByFacets.map((agent) => [agent.id, agent]));

    if (oramaRankedIds && oramaRankedIds.length > 0) {
      const ranked = oramaRankedIds
        .map((id) => filteredById.get(id))
        .filter((agent): agent is AiAgentPrompt => Boolean(agent));
    ...
    return filteredByFacets.filter((agent) => {
      const haystack = [agent.title, agent.role, agent.description, agent.category, ...agent.tags]
        .join(" ")
        .toLowerCase();
      return haystack.includes(normalizedLower);
    });
  }, [deferredQuery, filteredByFacets, oramaRankedIds]);

  const shouldVirtualize = searchedAgents.length > 30;

  const rows = useMemo(() => {
    const grouped: AiAgentPrompt[][] = [];
    for (let index = 0; index < searchedAgents.length; index += columnCount) {
      grouped.push(searchedAgents.slice(index, index + columnCount));
    }
    return grouped;
  }, [searchedAgents, columnCount]);
  ...
  }, [expandedAgentId, selectedPlatform, columnCount, rows.length, rowVirtualizer]);
  ...
  function renderAgentCard(agent: AiAgentPrompt) {
      ...
      >
        ...
        <div className="mt-4 rounded-xl border border-line/80 bg-slate-50/80 p-3 dark:border-slate-700 dark:bg-slate-900/60">
          ...
          {expanded ? (
            ...
            <p className="mt-2 text-xs leading-6 text-slate-700 dark:text-slate-200">{promptPreview(platformPrompt)}</p>
          )}
        </div>
    ...
    <div className="space-y-5">
      ...
      {searchedAgents.length === 0 ? (
        ...
        <section className="rounded-2xl border border-line/70 bg-white p-3 shadow-soft dark:border-slate-800 dark:bg-slate-950/70 sm:p-4">
          <div ref={listRef} className="max-h-[74vh] overflow-auto pr-1">
              ...
              {rowVirtualizer.getVirtualItems().map((virtualRow) => {
                ...
                return (
                  <div
                    ...
                    className={cx("absolute left-0 top-0 grid w-full gap-4", columnCount === 2 ? "grid-cols-2" : "grid-cols-1")}
                    style={{ transform: `translateY(${virtualRow.start}px)` }}
                  >
                    {row.map((agent) => renderAgentCard(agent))}
                  </div>
                );
              })}
            </div>
        ...
        <section className="grid gap-4 md:grid-cols-2">{searchedAgents.map((agent) => renderAgentCard(agent))}</section>
      )}
    </div>
  );
}
